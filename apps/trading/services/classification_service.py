...
def _parse_v2_extraction_response(raw: str) -> dict:
    ...
    for index, p in enumerate(data.get('products') or []):
        ...
        products.append({
            ...
            'is_sku_like': bool(p.get('is_sku_like')),
            'sku_code': str(p.get('sku_code') or '').strip(),
            'inferred_product_name': str(p.get('inferred_product_name') or '').strip(),
        ...
        'products': [
            {
                ...
                'is_sku_like': bool(product.get('is_sku_like')),
                'sku_code': product.get('sku_code') or '',
                'inferred_product_name': product.get('inferred_product_name') or '',
