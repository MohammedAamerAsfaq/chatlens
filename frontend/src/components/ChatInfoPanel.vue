...
watch(() => [props.open, store.selectedChatId], async ([open, chatId]) => {
  if (!open || !chatId) { info.value = null; return }
  loading.value = true
  try {
  ...
  <div class="w-80 h-full flex flex-col bg-white border-l border-gray-200 overflow-hidden">
    ...
    <div v-else class="flex-1 overflow-y-auto">
      ...
      <div class="flex flex-col items-center px-6 py-8 bg-white border-b border-gray-100">
        <!-- Group avatar -->
        <div v-if="isGroup" class="w-24 h-24 rounded-full bg-gray-300 flex items-center justify-center mb-4">
        ...
        >
          {{ avatarLetter(info.display_name) }}
        </div>

        <p class="text-lg font-semibold text-gray-900 text-center">{{ info.display_name }}</p>

        <!-- Phone number — shown for all contact types when resolved -->
        <p v-if="info.contact?.phone_number" class="text-sm text-gray-500 mt-0.5">
          +{{ info.contact.phone_number }}
        </p>

        <!-- Privacy mode notice — only when @lid AND phone not yet resolved -->
        <p v-if="isLid && !info.contact?.phone_number" class="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-full px-3 py-1 mt-2">
          Phone not resolved — reconnect to try again
        </p>
      ...
      <!-- Contact name details (if different from display) -->
      <div v-if="info.contact && (info.contact.push_name || info.contact.display_name)" class="px-5 py-4 border-b border-gray-100 space-y-2">
      ...
      <div v-if="info.media_counts.total > 0" class="px-5 py-4 border-b border-gray-100">
        ...
        <div class="space-y-2">
          ...
          >
            <span class="flex items-center gap-2 text-gray-600 capitalize">
              <span>{{ icon }}</span> {{ type }}s
            </span>
