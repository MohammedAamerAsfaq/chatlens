          ...
          <div v-if="expandedInquiries.has(inquiry.id)" class="inquiry-detail">
            ...
            <div class="detail-supplier-list">
              ...
              <div v-for="(supplier, supplierIndex) in inquiry.suppliers" :key="supplier.id" class="supplier-row compact-row">
                ...
                <label class="chat-state-toggle" :class="{ existing: supplier.is_existing_chat }">
                  <input
                    type="checkbox"
                    :checked="supplier.is_existing_chat"
                    :disabled="busyAction === `chat-state-${supplier.contact}`"
                    @change="setContactChatState(supplier, $event)"
                  />
                  <span>{{ supplier.is_existing_chat ? 'Existing Chat' : 'New Chat' }}</span>
                </label>
...
async function setContactChatState(supplier, event) {
  const isExisting = event.target.checked
  busyAction.value = `chat-state-${supplier.contact}`
  error.value = ''
  try {
    const { data } = await contactsApi.update(supplier.contact, { is_existing_chat: isExisting })
    inquiries.value.forEach(inquiry => {
      inquiry.suppliers.forEach(row => {
        if (row.contact === supplier.contact) row.is_existing_chat = data.is_existing_chat
      })
    })
  } catch (exc) {
