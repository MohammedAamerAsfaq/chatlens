...
<template>
  <main class="page">
    <header><div><h1>Task &amp; Queues</h1><p>Control parallel background work and how long an AI task may wait before it fails safely.</p></div><button :disabled="loading || saving" @click="save">{{ saving ? 'Saving...' : 'Save Settings' }}</button></header>
    <p v-if="error" class="error">{{ error }}</p><p v-if="saved" class="saved">Saved. Running workers use the new limits on their next poll.</p>
    <section v-if="!loading" class="grid"><article><h2>Runtime Modes</h2><code>Database controlled</code><label v-for="field in ['automation_mode','embedding_mode','classification_mode','recovery_mode']" :key="field">{{ field.replaceAll('_', ' ') }}<select v-model="runtime[field]"><option value="thread">Thread</option><option value="db_queue">DB queue</option></select></label><label>Worker heartbeat (seconds)<input v-model.number="runtime.worker_heartbeat_seconds" type="number" min="1" max="300"></label><label>Scheduler interval (seconds)<input v-model.number="runtime.scheduler_interval_seconds" type="number" min="1" max="300"></label></article><article v-for="queue in queues" :key="queue.name"><h2>{{ queue.display_name }}</h2><code>{{ queue.name }}</code><label>Concurrent tasks<input v-model.number="queue.max_concurrency" type="number" min="1" max="64"></label><small>Tasks may run in parallel up to this limit.</small><label>AI task timeout (minutes)<input v-model.number="queue.task_timeout_minutes" type="number" min="1" max="120"></label><small>After this deadline, the task fails before it can apply late AI output.</small></article></section>
  </main>
</template>
