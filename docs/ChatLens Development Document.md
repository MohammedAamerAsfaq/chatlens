...
**Automatic recovery:** `IngestionService.recover_unresolved_for_lid(account, lid_jid, phone_jid)`, triggered from `internal_contacts_update` whenever contact metadata provides a `lid_jid` mapping from `contacts.set`/`contacts.upsert`. The endpoint collects all LID mappings from one received contact batch and starts **one** background recovery thread for the whole batch, rather than one thread per contact. The recovery thread calls `close_old_connections()` at start and `connection.close()` in `finally`, so replayed or large contact updates cannot leave idle PostgreSQL connections open. Each mapping still reprocesses every `pending` row for that `(account, lid_jid)` through the *same* `_upsert_contact`/`_upsert_chat`/`_insert_message` path normal ingestion uses (no separate business logic), then routes post-processing through the normal live-vs-history split (`_process_message_in_background` for live, `_embed_in_background` only for history — never live-classifying a resurfaced historical message).
